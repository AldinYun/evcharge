...
export function AirRankingTable({ regions, metric, title }: { regions: AirRegionMetric[]; metric: Metric; title: string }) {
  const value = (region: AirRegionMetric) => {
    if (metric === "avgPm10") return `${Math.round(region.avgPm10)}㎍/㎥`;
    if (metric === "avgPm25") return `${Math.round(region.avgPm25)}㎍/㎥`;
  ...
  return (
    <section className="rounded-lg border border-slate-200 bg-white p-4 shadow-soft">
          ...
          <tbody>
            {regions.map((region, index) => (
              <tr key={region.id} className="border-b border-slate-100 last:border-0">
                ...
                <td className="py-3 pr-3">{Math.round(region.avgPm10)}㎍/㎥</td>
                <td className="py-3 pr-3">{Math.round(region.avgPm25)}㎍/㎥</td>
                <td className="py-3">{region.ventilationStatus}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <div className="space-y-3 md:hidden">
        {regions.map((region, index) => (
          <Link key={region.id} href={`/region/${encodeURIComponent(region.sido)}`} className="block rounded-md border border-slate-200 p-3">
            ...
            <div className="mt-2 grid grid-cols-3 gap-2 text-xs text-slate-600">
              <span>PM10 {Math.round(region.avgPm10)}</span>
              <span>PM2.5 {Math.round(region.avgPm25)}</span>
              <span>{region.ventilationStatus}</span>
            </div>
          </Link>
        ))}
      </div>
