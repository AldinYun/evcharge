...
export const metadata: Metadata = {
  title: "오늘 환기해도 될까 | 전국 미세먼지·환기 가이드",
  description: "전국 미세먼지와 초미세먼지 관측값을 기반으로 지금 환기 가능 여부와 생활 점수를 계산합니다."
};

export default async function HomePage() {
  const data = await getDashboardDataset();
  const national = data.national;
  const bestVentilation = [...data.sigunguMetrics].sort((a, b) => b.ventilationScore - a.ventilationScore);
      ...
      <div className="mt-6 grid grid-cols-1 gap-6 lg:grid-cols-[minmax(0,2fr)_minmax(280px,1fr)]">
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
            <RegionInsightCard title="환기하기 좋은 시/구" regions={bestVentilation} metric="ventilationScore" />
            <RegionInsightCard title="초미세먼지 높은 시/구" regions={worstPm25} metric="avgPm25" />
          </div>
          <AirRankingTable title="전국 시도별 환기 점수" regions={sidoRanking} metric="ventilationScore" />
        </div>
