...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    ...
    description: `${label} 등급으로 분류된 전국 미세먼지 측정소와 PM10, PM2.5, 환기 점수를 확인합니다.`
  };
}
  ...
  return (
    <DashboardShell title={`${label} 대기질 측정소`} description="대기질 등급 분포에서 선택한 등급에 해당하는 전국 측정소 목록입니다.">
      <section className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <MetricCard label="평균 PM10" value={microgram(averagePm10)} />
        <MetricCard label="평균 PM2.5" value={microgram(averagePm25)} tone={averagePm25 > 35 ? "bad" : "default"} />
        ...
        <aside className="space-y-6">
          ...
          <section className="rounded-lg border border-slate-200 bg-white p-4 shadow-soft">
            ...
            <p className="mt-2 text-sm leading-6 text-slate-600">
              좋음은 PM10 30㎍/㎥ 이하이면서 PM2.5 15㎍/㎥ 이하, 보통은 PM10 80㎍/㎥ 이하이면서 PM2.5 35㎍/㎥ 이하 기준으로 분류합니다.
